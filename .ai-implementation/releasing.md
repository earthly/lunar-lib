...
4. **Verify the `Policy sync` workflow passes** in `pantalasa-cronos/lunar` — it runs `lunar hub pull` to push the updated manifest to the hub. If it fails, the hub won't pick up the new configuration.
