...
def min_compiler_version_cicd(min_version=None, node=None):
    """Ensures compiler version in CI meets minimum."""
    if min_version is None:
        min_version = variable_or_default("min_compiler_version", "12.0.0")

    c = Check("min-compiler-version-cicd", "Ensures CI compiler version meets minimum", node=node)
    with c:
        ...
        if not cmds_node.exists():
            c.skip("No compiler commands recorded in CI")
        ...
        # Filter for compiler commands (not cmake)
        compiler_cmds = [
            ci for ci in cmds
            if not ci.get("cmd", "").strip().startswith("cmake")
        ]
        if not compiler_cmds:
            c.skip("No compiler commands found in CI data")

        violations = []
        for cmd_info in compiler_cmds:
        ...
        if violations:
            c.fail(
                f"Compiler version(s) below minimum {min_version}:\n"
