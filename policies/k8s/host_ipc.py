...
def main(node=None):
    ...
    with c:
        ...
        for workload in workloads:
            ...
            host_ipc = workload.get_value_or_default(".host_ipc", False)

            c.assert_true(
                host_ipc is not True,
                f"{path}: {kind} {namespace}/{name} should not set spec.hostIPC: true (workload shares the host IPC namespace)"
            )
