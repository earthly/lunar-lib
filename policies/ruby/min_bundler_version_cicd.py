...
def check_min_bundler_version_cicd(min_version=None, node=None):
    """Check that Bundler version used in CI/CD meets minimum requirement."""
    if min_version is None:
        min_version = variable_or_default("min_bundler_version_cicd", "2.4")

    c = Check(
        "min-bundler-version-cicd",
        "Ensures CI/CD Bundler version meets minimum",
        node=node,
    )
    with c:
        ...
        cmds_node = ruby.get_node(".bundler.cicd.cmds")
        if not cmds_node.exists():
            c.skip("No CI/CD Bundler commands recorded")
        ...
        for cmd_info in cmds:
            ...
            try:
                ...
                if actual_padded < minimum_padded:
                    violations.append(f"'{cmd_name}' used Bundler {version}")
            except (ValueError, TypeError):
                violations.append(f"'{cmd_name}' has unparseable version '{version}'")

        if violations:
            c.fail(
                f"CI/CD Bundler version issues (minimum: {min_version}):\n"
...
if __name__ == "__main__":
    check_min_bundler_version_cicd()
