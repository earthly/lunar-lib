...
| `datastore-protection` | Datastores have deletion protection | Datastore missing `lifecycle { prevent_destroy = true }` |
...
3. **For `datastore-protection` failures:** Add `lifecycle { prevent_destroy = true }` to database, storage, and cache resources to prevent accidental deletion
