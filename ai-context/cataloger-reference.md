...
Runs on a schedule, once **per component**, with the component's most recent Component JSON available. Unlike `cron` (which runs once globally), `component-cron` triggers an independent cataloger run for each component, with `LUNAR_COMPONENT_ID` set and `lunar component get-json` returning that component's data.
...
**Context:** Lunar Runner, per-component execution. `LUNAR_COMPONENT_ID` is set. `lunar component get-json` returns the component's most recent Component JSON from previous collector runs.

**Use cases:**
- Classifying components based on collected data (production vs non-production, technology tags)
- Setting organizational tags based on CI patterns (docker push to prod registry → `production` tag)
- Deriving tags from Component JSON without collector/cataloger chain complexity

**The "one step behind" trade-off:** On the very first run for a new component, there's no prior Component JSON—so no classification. Collectors run unfiltered, produce data. That night, the `component-cron` cataloger reads it and classifies. Next run, tags are set and targeting works. One-day lag on day one, then steady state. Classification rarely changes, so the lag is acceptable.

**Example — "what is production" classifier:**

```bash
#!/bin/bash
set -e

# Hook: type: component-cron, schedule: "0 2 * * *"

JSON=$(lunar component get-json 2>/dev/null || echo "{}")

# Detect technologies from Component JSON
[ "$(echo "$JSON" | jq -r '.lang.go // empty')" ] && lunar catalog component --tag go
[ "$(echo "$JSON" | jq -r '.lang.python // empty')" ] && lunar catalog component --tag python
[ "$(echo "$JSON" | jq -r '.lang.nodejs // empty')" ] && lunar catalog component --tag nodejs
[ "$(echo "$JSON" | jq -r '.containers // empty')" ] && lunar catalog component --tag docker-img
[ "$(echo "$JSON" | jq -r '.k8s // empty')" ] && lunar catalog component --tag k8s
[ "$(echo "$JSON" | jq -r '.iac // empty')" ] && lunar catalog component --tag iac

# Detect production based on CI patterns
BUILDS=$(echo "$JSON" | jq -r '.containers.docker.cicd.cmds[]?.cmd // empty' 2>/dev/null)
if echo "$BUILDS" | grep -qE 'push.*prod|push.*production'; then
