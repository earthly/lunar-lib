...
# Hook: type: after-json, path: ".sbom"
# If no SBOM was produced by CI or any other collector, generate one.
EXISTING=$(lunar component get-json ".sbom" 2>/dev/null || echo "")
if [ -n "$EXISTING" ] && [ "$EXISTING" != "null" ]; then
  echo "SBOM already collected, skipping auto-generation"
  exit 0
fi

# No SBOM from any source — generate one
...
collectors:
  - name: main-collector              # Required: Unique within plugin
    ...
    hook:
      type: code                      # Or: cron, ci-*, after-json
