#!/bin/bash
# argocd-deployment-tracking: correlate each ArgoCD Application to the source component it deploys
...
# shellcheck source=collectors/argocd-deployment-tracking/parse.sh disable=SC1091
...
if [ -z "$CONN_STRING" ] || [[ "$CONN_STRING" == *"Error"* ]]; then
    echo "argocd-deployment-tracking: hub SQL connection unavailable, skipping." >&2
    exit 0
fi
if ! command -v psql >/dev/null 2>&1; then
    apk add --no-cache postgresql-client >/dev/null 2>&1 || { echo "argocd-deployment-tracking: psql unavailable." >&2; exit 0; }
fi
...
if [ "$APP_COUNT" -eq 0 ]; then
    echo "argocd-deployment-tracking: no ArgoCD Applications found, nothing to link." >&2
    exit 0
fi
...
for ((i=0; i<APP_COUNT; i++)); do
    ...
    if [ -z "$LINE" ]; then
        echo "argocd-deployment-tracking: could not resolve a source component for app '$NAME' — skipping." >&2
        continue
    fi
    ...
    if [ -z "$TARGET_ID" ] || [ -z "$TARGET_SHA" ]; then
        echo "argocd-deployment-tracking: app '$NAME' resolved but missing component/sha — skipping." >&2
        continue
    fi
    if [ "$TARGET_ID" = "$LUNAR_COMPONENT_ID" ]; then
        continue  # don't push onto the GitOps repo itself
    fi
    echo "argocd-deployment-tracking: app '$NAME' -> $TARGET_ID@${TARGET_SHA:0:8} (via $STRAT)" >&2
...
for ((g=0; g<GROUP_COUNT; g++)); do
    ...
    if [ -n "$HAVE_NAMES" ] && [ "$HAVE_NAMES" = "$WANT_NAMES" ]; then
        echo "argocd-deployment-tracking: $TID already linked from $LUNAR_COMPONENT_ID with the same app set — skipping (idempotent)." >&2
    ...
    if env -u LUNAR_COLLECT_STDOUT -u LUNAR_LOG_PREFIX lunar collect \
        ...
        echo "argocd-deployment-tracking: out-of-band push to $TID failed: $(cat /tmp/oob-err 2>/dev/null)" >&2
...
echo "argocd-deployment-tracking: pushed deployment posture to $PUSHED source component(s)." >&2
