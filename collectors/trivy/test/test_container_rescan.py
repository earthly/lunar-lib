...
class ProvenanceTest(Base):
    ...
    def with_repo_digests(self, repo_digests):
        ...
        results = json.loads(self.TRIVY_RESULTS)
        results["Metadata"]["RepoDigests"] = repo_digests
        self.fixture("trivy-results.json", json.dumps(results))
...
class ScanHistoryTest(Base):
    ...
    PRIOR_SCAN = {
        ...
        "images": [{"image": "earthly/lunar-hub:main-old", "tool": "trivy"}],
        "native": {"trivy": {"matches": [1, 2, 3]}},
    }
