...
| `container-scan` | after-json | Scans the just-**pushed** image **synchronously at publish time**: fires once the docker collector has recorded the CI docker commands (`.containers.native.docker.cicd.cmds[]`), resolves the pushed image, scans it in the Trivy collector image, and writes `.container_scan` (`integration: "after-json"`) |
