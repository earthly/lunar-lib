...
COMPILER_BIN="${LUNAR_CI_COMMAND_BIN_DIR:+$LUNAR_CI_COMMAND_BIN_DIR/}${LUNAR_CI_COMMAND_BIN:-gcc}"

# Get compiler version
version=$("$COMPILER_BIN" --version 2>/dev/null | head -1 | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "")

if [[ -z "$version" ]]; then
    version=$("$COMPILER_BIN" -dumpversion 2>/dev/null || echo "")
fi
