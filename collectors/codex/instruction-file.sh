...
# Discover CODEX.md instruction files.
# Writes to ai.native.codex.instruction_file and appends to ai.instructions.all[].

FILES=$(find . \( -type f -o -type l \) -name CODEX.md -not -path '*/node_modules/*' -not -path '*/.git/*' 2>/dev/null || true)

if [ -z "$FILES" ]; then
  jq -n '{ exists: false }' | lunar collect -j ".ai.native.codex.instruction_file" -
  exit 0
fi
...
jq -n \
  ...
  }' | lunar collect -j ".ai.native.codex.instruction_file" -
