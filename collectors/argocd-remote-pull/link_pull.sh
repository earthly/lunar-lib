#!/bin/bash
# argocd-remote-pull: the pull counterpart to argocd-remote-push. Runs on a SERVICE repo and
...
#
# Why pull instead of push: argocd-remote-push writes onto the source component at its
# default-branch HEAD sha, so the posture only ever lands on post-merge main —
# a PR check evaluating at the PR's head sha can't see it. argocd-remote-pull runs in the
...
# than run with a broken self-ref (mirrors argocd-remote-push).
...
if [ -z "$GITOPS_COMPONENT" ]; then
    echo "argocd-remote-pull: no '$GITOPS_ANNOTATION' mapping in catalog-info and no override — skipping." >&2
    exit 0
fi
if [ "$GITOPS_COMPONENT" = "$LUNAR_COMPONENT_ID" ]; then
    echo "argocd-remote-pull: gitops component == self ($LUNAR_COMPONENT_ID) — skipping." >&2
    exit 0
fi
...
GITOPS_JSON=$(env -u LUNAR_COLLECT_STDOUT -u LUNAR_LOG_PREFIX \
    ...
    echo "argocd-remote-pull: get-json '$GITOPS_COMPONENT' failed: $(head -c 300 /tmp/pull-err 2>/dev/null)" >&2
    exit 0
}

if ! echo "$GITOPS_JSON" | jq -e '.cd.gitops.applications' >/dev/null 2>&1; then
    echo "argocd-remote-pull: '$GITOPS_COMPONENT' has no .cd.gitops.applications — nothing to pull." >&2
    exit 0
fi
...
# append-not-idempotent behaviour argocd-remote-push guards against), so collapse repeats
...
if [ "$APP_COUNT" -eq 0 ]; then
    echo "argocd-remote-pull: '$GITOPS_COMPONENT' carries no Application matching [${APPLICATIONS:-<all>}] — skipping." >&2
    exit 0
fi
...
echo "argocd-remote-pull: materialized $APP_COUNT application(s) from $GITOPS_COMPONENT onto $LUNAR_COMPONENT_ID." >&2
