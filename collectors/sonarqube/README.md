...
component: `api-default` (per-commit Web API on the default branch),
`api-pr` (per-PR Web API, scoped with `pullRequest=`), `config` (detects
in-repo SonarQube configuration), `cicd` (captures `sonar-scanner` runs),
and `github-app` (reads SonarCloud's GitHub App PR check). Users who don't
gate PRs with SonarQube can simply exclude the `api-pr` (and `github-app`)
sub-collectors from `lunar-config.yml`. Results land in the tool-agnostic
`.code_quality` category, with SonarQube-specific structure stashed under
...
|------|------|-----------|-------------|
| `.code_quality.source` | object | `api-default` / `api-pr` | Tool, integration (`api-default` or `api-pr`), project key, API URL, and `analysis_status` (`complete` or `pending`) |
| `.code_quality.passing` | bool | `api-default` / `api-pr` | Overall pass/fail signal — derived from SonarQube's quality gate status |
| `.code_quality.coverage_percentage` | number | `api-default` / `api-pr` | Line coverage percentage (0–100), if measured |
| `.code_quality.duplication_percentage` | number | `api-default` / `api-pr` | Duplicated lines percentage (0–100), if measured |
| `.code_quality.issue_counts` | object | `api-default` / `api-pr` | Severity buckets: `total`, `critical`, `high`, `medium`, `low` (same shape as `.sca` / `.sast`) |
| `.code_quality.native.sonarqube.quality_gate` | object | `api-default` / `api-pr` | Quality gate status (`OK`/`WARN`/`ERROR`) and failed condition count |
| `.code_quality.native.sonarqube.ratings` | object | `api-default` / `api-pr` | SonarQube letter ratings (A–E) per dimension: reliability, security, maintainability, security review |
| `.code_quality.native.sonarqube.metrics` | object | `api-default` / `api-pr` | SonarQube-native metric names: bugs, vulnerabilities, code smells, lines of code |
...
Required secrets:
- `SONARQUBE_TOKEN` — SonarQube/SonarCloud user token with `Browse` permission on the target project (used by the `api-default` and `api-pr` sub-collectors). Sent as the HTTP Basic username with an empty password, per the SonarQube Web API convention.
...
several minutes. The `api-default` and `api-pr` sub-collectors handle this
race by polling `api/project_analyses/search` until the most recent
analysis's `revision` matches the current `head_sha`, or until
...
| `api_poll_timeout_seconds` | `180` | Total seconds `api-default`/`api-pr` wait for a SonarQube analysis matching `head_sha`. `0` disables polling. |
