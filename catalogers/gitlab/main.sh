...
while IFS= read -r group; do
    ...
    while :; do
        ...
        [ "$n" -eq 0 ] && break
        jq -c --arg g "$group" '.[] | {id, path_with_namespace, description, topics: (.topics // .tag_list // []), archived, visibility, default_branch, group: $g, namespace_kind: .namespace.kind, pending_deletion: (.marked_for_deletion_on != null), is_fork: has("forked_from_project"), forked_from: (.forked_from_project.path_with_namespace // null)}' \
...
if [ "$INCLUDE_PERSONAL_NAMESPACES" = "true" ]; then
    ...
    while :; do
        ...
        jq -c '.[] | select(.namespace.kind == "user")
               | {id, path_with_namespace, description, topics: (.topics // .tag_list // []), archived, visibility, default_branch, group: .namespace.full_path, namespace_kind: .namespace.kind, pending_deletion: (.marked_for_deletion_on != null), is_fork: has("forked_from_project"), forked_from: (.forked_from_project.path_with_namespace // null)}' \
