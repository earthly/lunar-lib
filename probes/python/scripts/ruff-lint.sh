#!/bin/sh
# ruff-lint.sh — agent-after-file-edit check for the python.ruff-lint sub-probe.
#
...
# and runs `ruff check` against it. Exit non-zero on findings so the
# probe fires and the agent sees Ruff's diagnostic via `{check_stdout}`
# in the manifest's `message:` template. stderr is folded into stdout so
# parse/config errors surface there too.
#
