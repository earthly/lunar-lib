#!/bin/sh
# scripts/shellcheck.sh — agent-after-file-edit check for the shell.shellcheck sub-probe.
#
...
# folded into stdout so parse/IO errors surface there too.
#
# `shellcheck` presence is owned by the manifest's `requires:` block,
# which surfaces a missing linter once at session-end rather than
# skipping silently. The `command -v shellcheck` guard below is
# defense-in-depth for standalone invocation (e.g. running this script
# outside lunar-probe).
#
# Skip-safe (exit 0, edit proceeds) when:
#   - jq isn't on PATH (can't parse the payload).
#   - shellcheck isn't on PATH (standalone safety net; requires: handles the agent flow).
